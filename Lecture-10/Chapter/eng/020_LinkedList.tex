...
\begin{frame}{Linked Lists}{Implementation - Remove}
  ...
  \begin{itemize}
    ...
    \item<3->
      \texttt{cur} will get destroyed automatically if no more references exist
      (\lstinline[
        language=Python,
        style={python-idle-code},
        morekeywords={None}
...
\begin{frame}[fragile]{Linked Lists}{Implementation - Get}
  ...
  \begin{itemize}
    \item
      Iterate the entries of the list until position in $O(n)$
  \end{itemize}
  \onslide<2->
  \lstinputlisting[
    language=Python,
...
\begin{frame}{Linked Lists}{Doubly Linked List}
  ...
  \begin{itemize}
    ...
    \item<3->
      We only need {\color{Mittel-Blau}one head} node if we cyclically connect the list
  \end{itemize}
  \onslide<4->
  \begin{flushleft}
