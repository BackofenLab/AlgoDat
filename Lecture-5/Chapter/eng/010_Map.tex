...
\begin{frame}{Associative Arrays}{How do we build a Map?}
  ...
  \onslide<3->\item Naive solution: Store pairs of key and value
    in a normal field
  \onslide<4->\item For {\color{Mittel-Blau} n } keys searching requires {\color{Mittel-Blau} $\Theta(n)$} time
  \onslide<5->\item With a {\color{Mittel-Blau} Hash Map } this just requires {\color{Mittel-Blau} $\Theta(1)$}
