...
\draw (-1.25, 11.0) node[align=center](U_Text) {\Huge Key universe};
\draw[text=Mittel-Blau] (-1.25, 10.0) node[align=center](U) {\Huge$\mathbb{U}$};

\draw (2.0, 8.5) node[anchor=west](T_Text) {\Huge Hashtable};
\draw[text=Mittel-Blau] (4.0, 7.5) node[align=center](T) {\Huge T};
...
\foreach \y [count=\yi] in {5, 1, -8}{
  \draw[key_selected]
    ...
    (-1.5, 0.5*\y + 4.75) -- (-1.0, 0.5*\y + 4.75) --
    (-1.0, 0.5*\y + 5.25);
  \draw[text=Mittel-Blau] (-1.75, 0.5*\y + 5.0) node[anchor=east]
