...
% Bounding box
\draw[color=white] (-4, 0.5) rectangle (10, -4.25);
...
\only<4- |handout:1>{
  % General
  \node[lbl] (l5) at ($(4.5,0 |- c8) + (0,0 |- c8) - (0,0 |- n6)$)
    {Depth $d \rightarrow 2^{d-1}$ nodes};
  \node[lbl, anchor=east, align=right] (l6) at (l5.west) {Generally:};
}
\end{tikzpicture}