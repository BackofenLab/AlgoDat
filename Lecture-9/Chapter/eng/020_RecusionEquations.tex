...
\begin{frame}{Recursion Equations}{Recursion Equation}
  ...
  \begin{itemize}
    ...
    \end{displaymath}
    \item<3->
      {\color{MainA}$n_0$} is normally small, {\color{MainA}$f_0(n_0) \; \in \; \Theta(1)$}
    \item<4->
      Normally {\color{MainA}$a > 1$} and {\color{MainA}$b > 1$}
    \item<5->
...
\begin{frame}{Recursion Equations}{Recursion Tree Method Costs}
  ...
  \begin{itemize}
    \item<2->
      ...
      \end{math}
    \item<3->
      Costs of partial problem on {\color{MainA}level $i$}:
      \begin{displaymath}
        \color{MainA}{
          T_{i_p}(n) =
...
\begin{frame}{Fun with logarithm}{Logarithm}
  \begin{itemize}
  \item
    Transforming $\color{MainA}3^{\log_4 n}$ uses general log rules
    {\color{MainA}
    \begin{align*}
      \log_4 n & = \log_4 \left(3^{\log_3 n} \right) &&
      \qquad \text{\color{black}uses } n= 3^{\log_3 n}\\
      \onslide<2->{& = {\log_3 n \cdot \log_4 3} &&
        \qquad \text{\color{black}uses } \log a^b = b \cdot \log a}
    \end{align*}}
  \item<3->
...
\color{MainA}
    \begin{align*}
      ...
      \qquad \text{\color{black}uses reformulation above}\\
      \onslide<5->{& = {\left(3^{\log_3 n}\right)}^{\log_4 3} &&
        \qquad \text{\color{black}uses }x^{a \cdot b} = (x^a)^b}\\
      \onslide<6->{& = n^{\log_4 3}}
    \end{align*}
  \item<7->
