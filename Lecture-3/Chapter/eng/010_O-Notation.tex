...
\begin{frame}{$\mathcal{O}$-Notation}{Definition}
  ...
  \tikz[baseline] {\node (funtext) [anchor=north] {
    {\footnotesize\onslide<2- |handout:1>{\begin{tabular}[c]{@{}c@{}}
          ``set of\\
          all functions''
        \end{tabular}}
  }};}\quad{} \tikz[baseline] {\node (fortext) [anchor=north] {
    {\footnotesize\onslide<3- |handout:1>{\begin{tabular}[c]{@{}c@{}}
        ``for which''
        \end{tabular}}
  }};}\quad{} \tikz[baseline] {\node (extext) [anchor=north] {
    {\footnotesize\onslide<4- |handout:1>{\begin{tabular}[c]{@{}c@{}}
        ``it exists''
        \end{tabular}}
  }};}\quad{} \tikz[baseline] {\node (foralltext) [anchor=north] {
    {\footnotesize\onslide<5- |handout:1>{\begin{tabular}[c]{@{}c@{}}
        ``for all''
        \end{tabular}}
  }};}\quad{} \tikz[baseline] {\node (suchtext) [anchor=north] {
    {\footnotesize\onslide<6- |handout:1>{\begin{tabular}[c]{@{}c@{}}
        ``such that''
        \end{tabular}}
  }};}
  \begin{tikzpicture}[>=stealth,overlay,thick]
    \path[->,color=red]<2- |handout:1> (funtext) edge [bend left]  (fun);
    \path[->,color=red]<3- |handout:1> (fortext) edge [bend left]  (for);
    \path[->,color=red]<4- |handout:1> (extext) edge [bend left]  (exone);
    \path[->,color=red]<4- |handout:1> (extext) edge [bend right]  (extwo);
    \path[->,color=red]<5- |handout:1> (foralltext) edge [bend left]  (forall);
    \path[->,color=red]<6- |handout:1> (suchtext) edge [bend left]  (such);
  \end{tikzpicture}
\end{frame}
